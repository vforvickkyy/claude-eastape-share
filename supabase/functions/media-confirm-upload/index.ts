...
async function presignPut(endpoint: string, bucket: string, key: string, accessKeyId: string, secretAccessKey: string, region: string): Promise<string> {
  const now = new Date()
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: media, error: mediaErr } = await supabase
      .from('project_media')
      .select('wasabi_thumbnail_key, user_id, project_id, name')
      .eq('id', id)
      .eq('user_id', user.id)
      .single()

    if (mediaErr || !media) return json({ error: 'Media not found or not authorized' }, 404)

    // Upload thumbnail
    if (thumbnail_base64 && media.wasabi_thumbnail_key && ENDPOINT && BUCKET && ACCESS && SECRET) {
      try {
        const thumbnailBuffer = Uint8Array.from(atob(thumbnail_base64), c => c.charCodeAt(0))
        const putUrl = await presignPut(ENDPOINT, BUCKET, media.wasabi_thumbnail_key, ACCESS, SECRET, REGION)
        await fetch(putUrl, { method: 'PUT', headers: { 'Content-Type': 'image/jpeg' }, body: thumbnailBuffer })
      } catch { /* non-fatal */ }
    }

    const { data: updated, error } = await supabase
      .from('project_media')
      .update({ wasabi_status: 'ready', duration: duration || null, width: width || null, height: height || null, updated_at: new Date().toISOString() })
      .eq('id', id)
      .eq('user_id', user.id)
      .select()
      .single()
    ...
    await supabase.from('project_activity').insert({
      project_id: media.project_id, user_id: user.id, action: 'uploaded_media',
      entity_type: 'project_media', entity_id: id, entity_name: media.name,
    })

    return json({ asset: updated, media: updated })
  } catch (err) {
