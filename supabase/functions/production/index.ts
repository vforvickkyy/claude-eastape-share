...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (resource === 'shots_with_media') {
      ...
      // Batch fetch linked media names only — thumbnail URLs are generated by the frontend
      // using the same project-media endpoint that the Files tab uses.
      ...
      const shotsWithMedia = (shots || []).map((shot: any) => {
        ...
        if (!mid || !mediaMap[mid]) return { ...shot, linked_media_id: null, linked_media_name: null }
        return { ...shot, linked_media_id: mid, linked_media_name: mediaMap[mid] }
      })
