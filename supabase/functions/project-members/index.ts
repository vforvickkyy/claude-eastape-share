  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { data: members, error } = await supabase
        ...
        .select('*, profiles:user_id(full_name, avatar_url, email:id)')
    ...
    if (req.method === 'POST') {
      ...
      if (!project_id || !email) return json({ error: 'project_id and email required' }, 400)
      ...
      const { data: member, error } = await supabase.from('project_members').insert({
        project_id, user_id: profile?.id || null, invited_email: email,
        role: role || 'viewer', accepted: !!profile?.id, invited_by: user.id,
      }).select().single()
      if (error) return json({ error: error.message }, 500)
    ...
    if (req.method === 'PUT') {
      ...
      const { data: updated, error } = await supabase.from('project_members').update({ role: body.role }).eq('id', memberId).select().single()
      if (error) return json({ error: error.message }, 500)
