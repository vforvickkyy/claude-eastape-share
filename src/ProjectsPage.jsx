...
import {
  ...
  DotsThree, PencilSimple, Trash, CaretDown, X, Check,
  FolderOpen, Clock, Users,
} from "@phosphor-icons/react";
import { useAuth } from "./context/AuthContext";
import DashboardLayout from "./DashboardLayout";
import { projectsApi, formatSize } from "./lib/api";

const STATUS_OPTS = ["active", "archived", "completed", "on_hold"];
...
export default function ProjectsPage() {
  ...
  const [newIcon,   setNewIcon]   = useState(ICON_OPTS[0]);
  ...
  async function handleCreate(e) {
    ...
    setCreating(true);
    try {
      const d = await projectsApi.create({ name: newName.trim(), color: newColor, icon: newIcon, client_name: newClient });
      const created = d.project;
      setShowNew(false);
      setNewName(""); setNewClient(""); setNewColor(COLOR_OPTS[0]); setNewIcon(ICON_OPTS[0]);
              ...
              >
                ...
                <div className="project-card-body">
                  ...
                  <div className="project-card-meta">
                    {p.due_date && (
                      <span><Clock size={12} /> {new Date(p.due_date).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
              </motion.div>
