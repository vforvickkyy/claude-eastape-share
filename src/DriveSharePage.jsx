...
// ── Whole-project share view (flat media grid + player + comments) ────────────
function ProjectShareView({ rootData, token }) {
  const { project, assets = [], allowDownload, allowComments } = rootData
  ...
  function navFile(dir) {
    const next = assets[activeIdx + dir]
    if (next) openFile(next, activeIdx + dir)
  }
        ...
        <div style={{ ...card, maxWidth: 980, width: '100%', alignSelf: 'flex-start' }}>
          ...
          <div className="share-folder-header">
            <div style={{
              ...
              background: project?.color || 'rgba(124,58,237,0.25)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <FolderSimple size={20} weight="duotone" color="#fff" />
            </div>
            <div className="share-folder-header-info">
              <h1 style={{ fontSize: 19, fontWeight: 700, margin: '0 0 2px' }}>{project?.name || 'Shared project'}</h1>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.38)', margin: 0 }}>
            ...
            <div className="share-folder-actions">
              <ViewToggle view={viewMode} onChange={setViewMode} />
              {allowDownload && assets.length > 0 && (
                <button className="btn-ghost" style={{ fontSize: 12, height: 32, padding: '0 12px' }}
                  onClick={() => assets.forEach((f, i) => f.downloadUrl && setTimeout(() => triggerDownload(f.downloadUrl, f.name), i * 200))}>
                  <DownloadSimple size={13} /> Download all
                </button>
              )}
            </div>
          ...
          <div style={{ padding: '16px 20px 24px' }}>
            {assets.length === 0 ? (
              <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.28)', fontSize: 14, padding: '32px 0' }}>This project has no media yet.</p>
            ) : viewMode === 'list' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {assets.map((f, idx) => (
                  <div key={f.id} className="share-file-row" onClick={() => f.downloadUrl && openFile(f, idx)} style={{ cursor: f.downloadUrl ? 'pointer' : 'default' }}>
      ...
      {activeFile && (
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', minWidth: 40, textAlign: 'center' }}>{activeIdx + 1}/{assets.length}</span>
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.55)', padding: '4px 6px', borderRadius: 5, display: 'flex', alignItems: 'center' }} disabled={activeIdx >= assets.length - 1} onClick={() => navFile(1)}
              onMouseEnter={e => { if (activeIdx < assets.length - 1) e.currentTarget.style.color = '#fff' }}
              onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.55)'}>
              <CaretRightIcon size={16} />
            </button>
            <span style={{ fontWeight: 600, fontSize: 13, flex: 1, textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#fff' }}>{activeFile.name}</span>
...
export default function DriveSharePage() {
  ...
  // ── Whole-project view (flat media grid + player + comments) ─────────────
  if (rootData.type === 'project') {
    return <ProjectShareView rootData={rootData} token={token} />
  }
