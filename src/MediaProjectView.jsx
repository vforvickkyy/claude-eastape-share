...
/* ── Folder Share Modal ──────────────────────────────────────────── */
function FolderShareModal({ folder, onClose }) {
  ...
  useEffect(() => {
    // Fetch all share links and filter by folder_id client-side
    userApiFetch('/api/media/share')
      .then(d => setLinks((d.links || []).filter(l => l.folder_id === folder.id)))
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [folder.id]);
  ...
  async function generate() {
    ...
    try {
      const data = await userApiFetch("/api/media/share", {
        method: "POST",
        body: JSON.stringify({ folderId: folder.id, allowDownload, expiresAt: expiresAt || null, password: password || null }),
      });
  ...
  function getUrl(token) { return `${window.location.origin}/media/share/${token}`; }
      ...
      >
        ...
        <div className="share-modal-options">
          ...
          <div className="form-group" style={{ marginTop: 8 }}>
            ...
            <input className="form-input" type="password" placeholder="Leave blank for no password" value={password} onChange={e => setPassword(e.target.value)} />
          </div>
          <div className="form-group" style={{ marginTop: 8 }}>
            ...
            {links.map(l => (
              <div key={l.id} className="share-link-entry">
                <div className="share-link-row">
                  <input className="share-input" readOnly value={getUrl(l.token)} onFocus={e => e.target.select()} />
                  <button className={`copy-btn ${copied === l.token ? "copied" : ""}`} onClick={() => copy(l.token)}>
                ...
                <div className="share-link-analytics">
                  ...
                  {l.allow_download ? <span style={{ color: "var(--t3)" }}>Download on</span> : <span style={{ color: "var(--t3)" }}>View only</span>}
                  {l.last_accessed_at && <span>Last viewed {timeAgo(l.last_accessed_at)}</span>}
                  {l.expires_at && (
