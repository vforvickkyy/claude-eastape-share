...
export default function VersionsPanel({ asset, onVersionUploaded }) {
  ...
  async function handleVersionUploaded(newAsset) {
    try {
      // Bump version, transfer new Wasabi keys to original asset
      const d = await userApiFetch(`/api/media/assets?id=${asset.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          version_bump:          true,
          wasabi_key:            newAsset.wasabi_key,
          wasabi_thumbnail_key:  newAsset.wasabi_thumbnail_key,
          wasabi_status:         newAsset.wasabi_status,
