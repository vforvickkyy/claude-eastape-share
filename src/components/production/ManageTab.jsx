...
import { ListBullets, CalendarBlank, ChartBar, SlidersHorizontal, Plus, Spinner } from '@phosphor-icons/react'
...
export default function ManageTab() {
  ...
  if (statuses.length === 0 && shots.length === 0 && !loading) return (
    <div className="manage-empty">
      ...
      <button className="btn-primary" onClick={handleSeed} disabled={seeding}>
        {seeding ? 'Setting up…' : 'Get Started'}
      </button>
    </div>
  )

  const sharedProps = { projectId, statuses, scenes, columns, shots, onShotCreate: createShot, onShotUpdate: updateShot, onShotDelete: deleteShot, onSceneCreate: createScene, onReload: load }
